...
import { IS_WHITE_TURN, IS_BLACK_TURN, INITIAL_WHITE_PIECES_POS, INITIAL_BLACK_PIECES_POS } from "./properties"
...
function isLegalNonWinningMove(extendedState, eventData, settings){
  ...
  // Undo only if the move was legal, otherwise there is nothing to undo!!
...
function displayInitScreen(extendedState, eventData, settings) {
  const { draggable, width, position, boardStyle, squareStyles, turn, status } = extendedState;
...
// TODO summary
// change and retest web component? or leave like that: leave like that: NO change it if I change
// react-state-driven! do it now for 1.0!! hopefully little to no test to change but I have to test and change demos
// on codesandbox
// TODO: also address transducer.es.min.js:2145 Uncaught unknown event CLICKED found in state machine definition!
// this should be a warning, not an error unless detected by contract
